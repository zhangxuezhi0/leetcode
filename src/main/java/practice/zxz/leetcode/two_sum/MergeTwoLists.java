...
public class MergeTwoLists {
    ...
    public static ListNode mergeTwoLists(ListNode l1, ListNode l2) {
        ...
        while (l1 != null && l2 != null) {
            if (l1.val <= l2.val) {
                //这里不需要new新的节点，可以节省空间；不需要担心l1后面的所有节点都加入到了新链表上，对新链表产生的影响，
                //注意，这种做法有个副作用，就是会改变原来的l1,l2的数据，
//                result.next = l1;
...
//                result.next = l2;
     ...
     *       而这种数据结构，如果被类似递归的方法重新调用的话，可能产生两个链表互相引用的情况，形成引用环，那在遍历两个链表时，就会陷入死循环，最后导致内存耗尽系统崩溃。
     *
     * @param l1
     * @param l2
